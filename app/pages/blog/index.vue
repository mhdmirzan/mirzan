...
const allowedBlogSlugs = [
  "1-my-first-blog-post",
  "my-first-blog-post",
  "interveuu",
  "5-system-design-for-ai-engineers",
]
const allowedBlogTitles = [
  "My First Blog Post | Blog",
  "Transforming Recruitment with Interveuu",
...
const { data: blogData } = await useAsyncData(
  ...
  async () => {
    ...
    const filteredPosts = (allPosts || []).filter((post) => {
      ...
      const postStem = post.stem || ""
      const postTitle = post.title || ""

      return (
        allowedBlogSlugs.some(
          (slug) => postPath.includes(`/${slug}`) || postStem === slug,
        ) || allowedBlogTitles.includes(postTitle)
      )
    })

    const count = filteredPosts.length
    const start = (currentPage.value - 1) * itemsPerPage
    const posts = filteredPosts.slice(start, start + itemsPerPage)
